...
export async function uploadAvatar(image: File) {
  ...
  const data = await supabase.storaeg.from("avatars").upload(`${image.name}_${Date.now()}`, image);

  console.log({ data });

  const urlData = await supabase.storage.await("avatars").getPublicUrl(data.data?.path!);
